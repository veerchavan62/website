...
export default function LogoMarquee() {
  return (
    <section className="bg-neutral-50 py-20 md:py-24 border-b border-neutral-100">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="marquee-track flex items-center gap-10 md:gap-14">
          {doubled.map((brand, i) => (
            <div
              key={`${brand.name}-${i}`}
              className="flex-shrink-0 flex items-center justify-center w-32 md:w-36 h-14 md:h-16 bg-white rounded-lg border border-neutral-100 shadow-sm hover:shadow-md hover:border-neutral-200 transition-all duration-300"
            >
              <img
                src={brand.logo}
                alt={brand.name}
                className="h-5 md:h-6 w-auto object-contain max-w-[90px] md:max-w-[110px] opacity-70 hover:opacity-100 transition-opacity"
                loading="lazy"
              />
            </div>
          ))}
